...
function POMDPs.action(p::ToEnd, b::ParticleCollection{RoombaState})
    ...
    if previousBumpState == true
        ...
        for i = 1:numActions
            bp1=POMDPs.update(belief_updater, b,RoombaAct(policy.action_map[i]),AA228FinalProject.wall_contact(m_discrete,s))
            for j = 1:num_particles
                s1 = particle(bp1,j)
            end
            bp2=POMDPs.update(belief_updater, b,RoombaAct(policy.action_map[i]),AA228FinalProject.wall_contact(m_discrete,s1))
